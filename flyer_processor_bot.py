...
def capture_pages_prima5_pdf(issuu_url, store_name, count=4):
    ...
    if doc_id_match:
        ...
        try:
            api_url = f"https://reader3.isu.pub/{doc_id}/reader3-4.json"
    ...
    try:
        ...
        for pattern in [
            r'"(?:pdfUrl|pdf_url|downloadUrl|originalPdfUrl)"\s*:\s*"([^"]+\.pdf[^"]*)"',
            r'(https://[^"<>\s]+\.pdf)',
        ]:
...
def interpret_image_with_ai(image_path, page_num, store_name, title_name, link_hint, pre_calc_date=None, need_vision_pagenum=False, double_page_info=None):
    ...
    with open(image_path, "rb") as f:
        image_data = base64.b64encode(f.read()).decode("utf-8")
    image_content = {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{image_data}"}}
...
---"""
    ...
    resp1 = client.chat.completions.create(
        ...
        messages=[{"role": "user", "content": [image_content, {"type": "text", "text": prompt1}]}]
    )
    time.sleep(1)
...
TERMEK: [nev] | AR: [ar] Ft | KISZERE: [kiszereles] | EGYSEGAR: [egysegar] | FELTETEL: [ar feltetel] | LEIRAS: [leiras]"""
    ...
    # ============================================================
    prompt2 = f"""Ez egy magyar akcioS ujsag kepe. Egy elozetes elemzes mar azonositott termekeket:
...
ELVART JSON:
{{
  "ervenyesseg": "EEEE.HH.NN. - EEEE.HH.NN. vagy N/A",
  "oldalszam": {page_num},
  "termekek": [
    {{
      "nev": "marka + termeknev",
    ...
    resp2 = client.chat.completions.create(
        model="gpt-4o", temperature=0, max_tokens=8000,
        response_format={"type": "json_object"},
        messages=[
            {"role": "user", "content": [image_content, {"type": "text", "text": prompt2}]}
        ]
    )
    time.sleep(1)
    try:
...
Ha tenyleg nincs sehol ar, add vissza ures listat: {{"termekek": []}}"""
        ...
        for termek in termekek:
            ...
            if not validacio['ar_valid']:
                kiszurt += 1
                print(f"      ❌ Kiszűrve: {termek.get('nev', '?')}")
                continue
            for msg in validacio.get('figyelmeztetesek', []):
                print(f"      ⚠️ {msg}")
            validalt.append(javitott)
        if kiszurt > 0:
            print(f"   🔍 Validáció: {kiszurt} kiszűrve, {len(validalt)} maradt")
        result['termekek'] = validalt
        return result
...
def validalj_termeket(termek):
    ...
    - Irreális árak kiszűrése
    - Egységár kiszámítás ha hiányzik
    - Figyelmeztetések generálása
    """
    validacio = {'ar_valid': True, 'figyelmeztetesek': []}

    # Ár kinyerése
    ar_str = str(termek.get('ar', '') or '')
    ar_szam = None
    ar_match = re.search(r'[\d\s]+', ar_str.replace(' ', ''))
    if ar_match:
        try:
            ar_szam = float(ar_match.group().replace(' ', ''))
        except:
            pass

    # Irreális ár kiszűrése
    if ar_szam is not None:
        if ar_szam < 5 or ar_szam > 100000:
            validacio['ar_valid'] = False
            validacio['figyelmeztetesek'].append(
                f"Irreális ár kiszűrve: {ar_szam} Ft — {termek.get('nev', '?')}"
            )
            return termek, validacio

    # Egységár kiszámítás ha hiányzik
    if ar_szam and not termek.get('ar_egyseg'):
        kisz_str = str(termek.get('kiszereles', '') or '')
        kisz_match = re.search(r'([\d.,]+)\s*(kg|g|l|ml|dl)', kisz_str.lower())
        if kisz_match:
            try:
                kisz_szam = float(kisz_match.group(1).replace(',', '.'))
                kisz_egyseg = kisz_match.group(2)
                # Normalizálás kg/l-re
                if kisz_egyseg == 'g':
                    kisz_szam /= 1000
                    kisz_egyseg = 'kg'
                elif kisz_egyseg == 'ml':
                    ...
                    kisz_egyseg = 'l'
                if kisz_szam > 0:
                    egysegar = ar_szam / kisz_szam
                    termek['ar_egyseg'] = f"[sz] {egysegar:.0f} Ft/{kisz_egyseg}"
                    validacio['figyelmeztetesek'].append(
                        f"Egységár kiszámítva: {termek['ar_egyseg']}"
                    )
            except:
                pass

    # Figyelmeztetés ha nincs ár
    if not ar_szam:
        validacio['figyelmeztetesek'].append(
            f"Hiányzó/ismeretlen ár: {termek.get('nev', '?')}"
        )

    return termek, validacio


def interpret_pdf_text_with_ai(pdf_text, page_num, store_name, title_name, link_hint, pre_calc_date=None):
    """
    CBA / CBA Príma natív PDF szöveg feldolgozása — Vision nélkül!
    1 GPT-4o hívás szöveges prompttal + Python validáció.
    """
    print(f"\n   📄 PDF szöveges pipeline: oldal {page_num} ({store_name})")

    if not pdf_text or len(pdf_text.strip()) < 20:
        print(f"   ⚠️ Üres PDF szöveg — kihagyva")
        return {"termekek": [], "ervenyesseg": "N/A", "oldalszam": page_num}

    print(f"   → {len(pdf_text)} karakter natív szöveg")

    date_instr = ""
    if page_num == 1:
        if pre_calc_date and pre_calc_date != "N/A":
            date_instr = f'DATUM: Már tudjuk: {pre_calc_date} — pontosan ezt írd az "ervenyesseg" mezőbe!'
        else:
            date_instr = f'DATUM: Keresd meg az érvényességi időt a szövegben. Formátum: "ÉÉÉÉ.HH.NN. - ÉÉÉÉ.HH.NN.". FALLBACK: {link_hint}'

    prompt = f"""Ez egy magyar akciós újság {store_name} áruháztól. Az alábbi szöveg a {page_num}. oldalból lett kinyerve PDF-ből:
...
ELVÁRT JSON:
{{
  "ervenyesseg": "ÉÉÉÉ.HH.NN. - ÉÉÉÉ.HH.NN. vagy N/A",
  "oldalszam": {page_num},
  "termekek": [
    {{
      "nev": "márka + terméknév",
    ...
    try:
        ...
        print(f"   PDF szöveges: {len(result.get('termekek', []))} termék strukturálva")
...
def process_images_with_ai(captured_data, flyer_meta, all_flyers, pre_calc_date=None):
    ...
    for item in captured_data:
        ...
        ) if item.get('image_path') is not None else interpret_pdf_text_with_ai(
            item.get('pdf_text', ''),
            effective_pagenum,
            store_name,
            flyer_meta['title'],
